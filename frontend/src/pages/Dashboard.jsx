...
const Dashboard = () => {
  ...
  useEffect(() => {
    const allBlogs = async () => {
      try {
        const res = await axios.get("https://quillverse.onrender.com/blog/all", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const removeBlog = async (blogId) => {
    try {
      const res = await axios.delete(
        `https://quillverse.onrender.com/blog/delete/${blogId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
                  ...
                  {blogs.map((blog) => (
                    <tr key={blog._id} className="text-center">
                      ...
                      <td className="border px-4 py-2">
                        <img
                          src={`https://quillverse.onrender.com/images/${blog.image}`}
                          alt={blog.title}
